...
class PortfolioRepository:
    ...
    def get_or_create_default_portfolio(self, user_id: int, name: str = "My Portfolio") -> Portfolio:
        existing = self.get_user_portfolio(user_id)
        if existing:
            return existing
        with get_connection() as conn:
            cur = conn.execute(
            ...
            return Portfolio(id=cur.lastrowid, user_id=user_id, name=name, updated_at=datetime.now())
    ...
    def replace_positions(self, portfolio_id: int, df: pd.DataFrame):
        cleaned = self._normalize_holdings_df(df)
        with get_connection() as conn:
